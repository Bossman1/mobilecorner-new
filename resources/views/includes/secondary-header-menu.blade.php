    ...
    >
        <div class="flex h-full">
            ...
            <div class="min-w-[280px] border-r border-slate-200 p-4 bg-white">
                <ul class="space-y-2">
                    @foreach(['მობილურები', 'ლეპტოპები', 'ტელევიზორები', 'აუდიო სისტემა', 'სამზარეულო', 'სმარტ საათები'] as $index => $category)
                        <li
                ...
                @foreach(['მობილურები', 'ლეპტოპები', 'ტელევიზორები', 'აუდიო სისტემა', 'სამზარეულო', 'სმარტ საათები'] as $index => $category)
                    <div class="menu-content absolute inset-0 opacity-0 invisible hidden transition-all duration-300" data-content="{{ $index }}">
                        ...
                        <div class="grid grid-cols-3 gap-3 p-[15px]">
                            <div class="col-span-3"> <x-line :text="'კატეგორიის სახელი'"  /> </div>
                            @for($i = 1; $i <= 6; $i++)
                                <a href="">
                                    <div
                                        class="relative border border-slate-200 rounded-xl hover:shadow-md transition p-2 flex flex-col items-center text-center bg-white">
                                        <img src="https://placehold.co/160x160" alt="product"
                                             class="rounded-md">
                                        <h4 class="text-[14px] font-custom-bold-upper text-slate-700 absolute bottom-[10px] left-[15px]">{{ $category }} {{ $i }}</h4>
                                    </div>
                                </a>
                            @endfor
                        </div>
